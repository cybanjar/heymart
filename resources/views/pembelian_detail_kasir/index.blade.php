...
<tbody></tbody>
  ...
  <div class="col-md-4">
    <form class="form form-horizontal form-pembelian" method="post" action="{{  route('pembelian_kasir.store') }} ">
      {{ csrf_field() }}
      <input type="hidden" name="idpembelian" value="{{ $idpembelian }}">
...
@include('pembelian_detail_kasir.produk')
@endsection
...
$(function(){
  ...
  table = $('.tabel-pembelian').DataTable({
     ...
     "processing" : true,
     "ajax" : {
       "url" : "{{ route('pembelian_detail_kasir.data', $idpembelian) }}",
       "type" : "GET"
     }
  }).on('draw.dt', function(){
    loadForm($('#diskon').val());
...
function addItem(){
  $.ajax({
    url : "{{ route('pembelian_detail_kasir.store') }}",
    type : "POST",
...
function loadForm(diskon=0){
  ...
  $.ajax({
       url : "pembelian_detail_kasir/loadform/"+diskon+"/"+$('.total').text(),
       type : "GET",
       dataType : 'JSON',
       success : function(data){
